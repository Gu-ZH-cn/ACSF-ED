...
class AFPN(nn.Module):
    def __init__(self,
        ...
        self.conv0 = BasicConv(in_channels[0], in_channels[0] , 1)
        self.conv1 = BasicConv(in_channels[1], in_channels[1] , 1)
        self.conv2 = BasicConv(in_channels[2], in_channels[2] , 1)

        self.body = nn.Sequential(
            BlockBody([in_channels[0] , in_channels[1] , in_channels[2]])
        )

        self.conv00 = BasicConv(in_channels[0], out_channels, 1)
        self.conv11 = BasicConv(in_channels[1], out_channels, 1)
        self.conv22 = BasicConv(in_channels[2], out_channels, 1)
        self.conv33 = nn.MaxPool2d(kernel_size=1, stride=2)
...
class BlockBody(nn.Module):
    def __init__(self, channels=[256, 256, 256]):
        ...
        self.downsample_scalezero1_2 = Downsample_x2(channels[1], channels[2])
        self.upsample_scaleone1_2 = Upsample(channels[2], channels[1], scale_factor=2)

        self.asff_scalezero1 = ASFF_2(inter_dim=channels[1])
        self.asff_scaleone1 = ASFF_2(inter_dim=channels[2])
    ...
    def forward(self, x):
        ...
        scalezero = self.asff_scalezero1(x2, self.upsample_scaleone1_2(x1))
        scaleone = self.asff_scaleone1(self.downsample_scalezero1_2(x2), x1)

        x2 = self.blocks_scalezero2(scalezero)
        x1 = self.blocks_scaleone2(scaleone)

        scalezero = self.asff_scalezero2(x2, self.upsample_scaleone2_2(x1), self.upsample_scaletwo2_4(x0))
        scaleone = self.asff_scaleone2(self.downsample_scalezero2_2(x2), x1, self.upsample_scaletwo2_2(x0))
        scaletwo = self.asff_scaletwo2(self.downsample_scalezero2_4(x2), self.downsample_scaleone2_2(x1), x0)

        scalezero = self.blocks_scalezero3(scalezero)
        scaleone = self.blocks_scaleone3(scaleone)
        scaletwo = self.blocks_scaletwo3(scaletwo)

        return scaletwo,scaleone, scalezero

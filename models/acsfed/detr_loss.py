...
from models.yowo.detr_matcher import HungarianMatcher
from models.yowo.detr_o2m_matcher import Stage2Assigner
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, matcher, weight_dict, losses, alpha=0.2, gamma=2.0, eos_coef=1e-4, num_classes=80, o2m_matcher_threshold=0.4, o2m_matcher_k=6, num_queries=300, use_indices_merge = False):
    ...
    def loss_labels_bce(self, outputs, targets, indices, num_boxes, log=True):
        ...
        target_classes_o = torch.cat([t["labels"][J] for t, (_, J) in zip(targets, indices)])
        targets = []
        for idx,ious in zip(target_classes_o,ious):
            idx = F.one_hot(idx.long(), self.num_classes)
            idx = idx*ious
            targets.append(idx)
        cls_targets = torch.cat(targets, 0)
        src_logits = src_logits.reshape(-1)
        loss = self.cls_lossf(src_logits,cls_targets)
    ...
    def loss_labels_vfl(self, outputs, targets, indices, num_boxes, log=True):
        ...
        loss *=  src_logits.shape[1] / num_boxes
    ...
    def forward(self, outputs, targets):
        ...
        o2o_indices_list.append(indices)     # TODO：NEW
        ...
        if 'o2m_outputs' in outputs:
            ...
            for loss in self.losses:
                l_dict = self.get_loss(loss, o2m_outputs, targets, indices, num_boxes)
                l_dict = {k: l_dict[k] * self.weight_dict[k] for k in l_dict if k in self.weight_dict}
                l_dict = {k + f'_o2m_outputs': v for k, v in l_dict.items()}
                losses.update(l_dict)
            ...
            if 'aux_outputs' in outputs['o2m_outputs']:
                for i, aux_outputs in enumerate(o2m_outputs['aux_outputs']):
                    ...
                    for loss in self.losses:
                        ...
                        l_dict = self.get_loss(loss, aux_outputs, targets, indices, num_boxes, **kwargs)
                        l_dict = {k: l_dict[k] * self.weight_dict[k] for k in l_dict if k in self.weight_dict}
                        l_dict = {k + f'_aux_o2m_outputs_{i}': v for k, v in l_dict.items()}
                        losses.update(l_dict)
        ...
        if 'dn_aux_outputs' in outputs['o2m_outputs']:
            assert 'dn_meta' in outputs, ''
            indices = self.get_cdn_matched_indices(outputs['dn_meta'], targets)
            num_boxes = num_boxes * outputs['dn_meta']['dn_num_group']

            for i, aux_outputs in enumerate(outputs['o2m_outputs']['dn_aux_outputs']):
                # indices = self.matcher(aux_outputs, targets)
                for loss in self.losses:
                    if loss == 'masks':
                        # Intermediate masks losses are too costly to compute, we ignore them.
                    ...
                    l_dict = self.get_loss(loss, aux_outputs, targets, indices, num_boxes, **kwargs)
                    l_dict = {k: l_dict[k] * self.weight_dict[k] for k in l_dict if k in self.weight_dict}
                    l_dict = {k + f'_dn_o2m_outputs_{i}': v for k, v in l_dict.items()}
                    losses.update(l_dict)
...
def build_criterion(args, cfg, num_classes):
    matcher = HungarianMatcher(cfg['matcher_weight_dict'], use_focal_loss = True, alpha = cfg['matcher_alpha'], gamma = cfg['matcher_gamma'])
    criterion = SetCriterion(matcher, cfg['weight_dict'], cfg['losses'], cfg['alpha'], cfg['gamma'],eos_coef=1e-4, num_classes=num_classes)
    return criterion
...
from utils.solver.ema import ModelEMA
...
def parse_args():
    ...
    parser.add_argument('--topk', default=40, type=int,
                        help='topk prediction candidates.') # topk is not really use, it may not use
...
def train():
    ...
    # ema
    ema = ModelEMA(model, 0.9999, 2000)
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        for iter_i, (frame_ids, video_clips, targets) in enumerate(dataloader):
            ...
            # target's coordinate transform(xyxy to cxcywh)
            for ti, target in enumerate(targets):
                targets[ti]['boxes'] = box_xyxy_to_cxcywh(target['boxes'])
            ...
            # inference
            outputs = model(video_clips, targets)
            ...
            if distributed_utils.is_main_process() and iter_i % 10 == 0:
                ...
                print_log(cur_lr, epoch,  max_epoch, iter_i, epoch_size, losses, loss_dict_reduced, t1-t0, accumulate)
        ...
        if epoch % args.eval_epoch == 0 or (epoch + 1) == max_epoch:
            eval_one_epoch(args, model_without_ddp, evaluator, epoch, path_to_save, max_epoch)


def eval_one_epoch(args, model_eval, evaluator, epoch, path_to_save, max_epoch):
    # check evaluator
    if distributed_utils.is_main_process():
        if evaluator is None:
            print('No evaluator ... save model and go on training.')
            
        elif evaluator.dataset == 'ava_v2.2' or (evaluator.dataset == 'ucf24' and (epoch + 1) == max_epoch): # ucf24'eval is too slow, we only eval at last epoch
            print('eval ...')
            # set eval mode
...
def print_log(lr, epoch, max_epoch, iter_i, epoch_size, loss_value,loss_dict, time, accumulate):
    # basic infor
    log =  '[Epoch: {}/{}]'.format(epoch+1, max_epoch)
    log += '[Iter: {}/{}]'.format(iter_i, epoch_size)
    log += '[lr: {:.6f}]'.format(lr[0])
    # loss infor
    log += '[loss: {:.2f}]'.format(loss_value)

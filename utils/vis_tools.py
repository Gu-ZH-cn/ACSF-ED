...
def plot_bbox_labels(img, bbox, label=None, cls_color=None, text_scale=0.4):
    ...
    # plot bbox
    cv2.rectangle(img, (x1, y1), (x2, y2), cls_color, 2) #TODO: 2
    ...
    for i, bbox in enumerate(bboxes):
        if scores[i] > vis_thresh:
            ...
            if len(class_names) > 1:
                # mess = '%s: %.2f' % (class_names[label], scores[i]) # TODO:THIS IS USUALLY USE
                mess = None
            else:
                cls_color = [255, 0, 0]
                mess = None
...
def vis_GroundTrouth(frame, scores, labels, bboxes, vis_thresh, class_names, class_colors):
    ts = 0.1  # TODO :0.4
    n = 0
    for i, bbox in enumerate(bboxes):
        if scores[i] > vis_thresh:
            label = int(labels[i])
            cls_color = class_colors
            n = n + 1
